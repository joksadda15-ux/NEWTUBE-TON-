    ...
    if (update.callback_query) {
        ...
        if (data === 'a_flags') {
            ...
            for (const u of flagged) {
                ...
                allDocs.forEach(d => {
                    groupText += `${d.isBanned ? '⛔' : '✅'} <code>${d._id}</code> @${d.telegramUsername || '?'} — ${d.wtcBalance || 0} WTC\n`;
                });
                groupText += `\n<i>Pick the account to KEEP — all others will be banned.</i>`;

                const keepButtons = allDocs.map(d => ({ text: `Keep @${d.telegramUsername || d._id}`, callback_data: `devban_keep_${d._id}_|_${allInGroup.join(',')}` }));
                const rows = keepButtons.reduce((acc, btn, i) => { if (i % 2 === 0) acc.push([]); acc[acc.length - 1].push(btn); return acc; }, []);
                rows.push([{ text: '✅ Clear All Flags (no ban)', callback_data: `devban_clearall_${allInGroup.join(',')}` }]);

                await tgSend(chatId, groupText, { reply_markup: { inline_keyboard: rows } });
            }
            return res.status(200).json({ ok: true });
        }

        // ── Device bulk-ban: keep one, ban the rest ──
        if (data.startsWith('devban_keep_')) {
            const rest = data.replace('devban_keep_', '');
            const [keepId, idsStr] = rest.split('_|_');
            const allIds = idsStr.split(',');
            const toBan = allIds.filter(id => id !== keepId);
            await users.updateMany({ _id: { $in: toBan } }, { $set: { isBanned: true } });
            await users.updateOne({ _id: keepId }, { $set: { multiAccountFlag: false } });
            await tgAnswerCallback(cb.id, `✅ Banned ${toBan.length} account(s), kept ${keepId}`, true);
            for (const id of toBan) {
                await tgSend(id, `⛔ Your account has been banned for using multiple accounts on the same device.`).catch(() => {});
            }
            return res.status(200).json({ ok: true });
        }

        // ── Device clear all flags (no ban) ──
        if (data.startsWith('devban_clearall_')) {
            const allIds = data.replace('devban_clearall_', '').split(',');
            await users.updateMany({ _id: { $in: allIds } }, { $set: { multiAccountFlag: false } });
            await tgAnswerCallback(cb.id, '✅ All flags cleared', true);
            return res.status(200).json({ ok: true });
        }
