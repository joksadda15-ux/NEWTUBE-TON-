...
export default async function handler(req, res) {
    ...
    if (update.callback_query) {
        ...
        if (data === 'bc_confirm') {
            ...
            fetch(`${APP_URL}/api/broadcastWorker?jobId=${job._id}&secret=${BOT_TOKEN_FOR_WORKER}`).catch((err) => {
                console.error('bc_confirm: failed to kick off broadcastWorker:', err.message);
            });
