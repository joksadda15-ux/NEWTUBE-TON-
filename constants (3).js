...
//
// `requiresClick: true` — ⚠️ NEW, requested for giga and monetag ("must
// click"). IMPORTANT — this flag is NOT yet enforced anywhere in
// api/earn.js. Implementing a real click requirement needs to be checked
// against what GigaPub's and Monetag's own SDKs actually expose (some ad
// formats only resolve their completion promise on a genuine click, in
// which case it's already enforced by the network itself; a publisher-side
// "did they click" check on a cross-origin ad iframe generally isn't
// something JS can observe at all, for the same reason none of our other
// networks can either). Flagging it here as a marker of INTENT so it isn't
// forgotten, not as something currently active — see the chat response for
// what's been checked so far.
export const AD_NETWORK_REWARDS = {
    adsgramDaily:   { reward: 15, dailyLimit: 10, minWatchSeconds: 10 },
    adsgramSpecial: { reward: 25, dailyLimit: 10, minWatchSeconds: 20 },
    // ⚠️ CHANGED — reward 12→10 (admin decision, "kombiye den"/reduce it).
    monetag:        { reward: 10, dailyLimit: 10, minWatchSeconds: 10, requiresClick: true },
    giga:           { reward: 15, dailyLimit: 15, minWatchSeconds: 8, requiresClick: true },
    ...
    // ⚠️ CHANGED — reward 8→5 (admin decision, "kombiye den"/reduce it).
    monetagPopup:   { reward: 5, dailyLimit: 5 },
