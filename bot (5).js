...
// ══════════════════════════════════════════════════════════════
// ⚠️ NEW — banned-Telegram-ID registry (see MongoDB cleanup update)
//
// A permanently-banned user's full `users` document now auto-deletes 60
// days after `bannedAt` (see models/schema.js's TTL index) to free up
// free-tier storage. But if we ONLY deleted the users doc, that Telegram ID
// could simply reopen the app after deletion and api/user.js's handleInit
// would see "no existing user" and happily create them a brand-new,
// un-banned account — silently undoing the ban.
//
// To prevent that, every ban/unban action here ALSO writes to a tiny,
// separate `bannedTelegramIds` collection (just an _id, no other fields —
// near-zero storage cost, no TTL, persists forever). api/user.js's
// handleInit checks this registry before creating a new account and
// refuses if the ID is present. Unbanning removes the ID from this
// registry too, so a genuinely-reinstated user can rejoin normally.
async function markBanned(db, ids) {
...
async function markUnbanned(db, ids) {
    const list = ids.filter(Boolean).map(String);
    if (!list.length) return;
...
export default async function handler(req, res) {
    ...
    if (update.callback_query) {
        ...
        if (data.startsWith('wd_approve_') || data.startsWith('wd_reject_')) {
            ...
            if (!approve) {
                // ⚠️ MAJOR FIX: withdrawals now deduct from `usdtBalance` (the
                // convert-first flow), not `wtcBalance` — this used to refund
                // the wrong balance entirely, meaning a rejected withdrawal's
                // money would just vanish instead of being returned to the
                // user. Now it refunds `cashAmount` (the USD amount actually
                // deducted) back into usdtBalance.
                // Tier claim-slot refund (unchanged from before): a rejected
                // withdrawal shouldn't permanently burn one of the user's
                // limited monthly claims for that tier.
                const refundUpdate = { $inc: { usdtBalance: w.cashAmount, withdrawalCount: -1 }, $set: { lastWithdrawDate: '' } };
                if (w.tierId) refundUpdate.$inc[`withdrawTierCounts.${w.tierId}`] = -1;
            ...
            const notif = approve
                ...
                : `❌ <b>Withdrawal Rejected.</b>\nYour $${w.cashAmount.toFixed(2)} has been refunded to your converted balance.`;
