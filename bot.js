...
import { tgApi, tgSend, tgEdit, tgAnswerCallback, isMember, OFFICIAL_CHANNEL, COMMUNITY_GROUP } from '../lib/telegram.js';
...
const APP_URL = 'https://newtube-ton.vercel.app';                       // আপনার মিনি অ্যাপের Vercel URL (যাচাই করে নিন ঠিক আছে কিনা)
const MINI_APP_URL = 'https://t.me/YOUR_BOT_USERNAME/YOUR_APP_NAME';    // TODO: আপনার বট-ইউজারনেম ও মিনি-অ্যাপের শর্ট-নেম দিয়ে বদলান
...
const adminKb = {
    inline_keyboard: [
        ...
        [{ text: '👤 User Lookup', callback_data: 'a_user' }, { text: '🚩 Multi-Acc Flags', callback_data: 'a_flags' }],
...
export default async function handler(req, res) {
    ...
    if (update.callback_query) {
        ...
        if (data.startsWith('check_join_')) {
            ...
            await tgSend(chatId, `✅ <b>Verified! Welcome to NEWTUBE!</b>\n\n🎬 Watch videos · Earn WTC · Withdraw crypto!`, {
        ...
        if (data === 'a_pending') {
            ...
            await tgEdit(chatId, msgId, `💸 <b>${list.length} pending withdrawal(s)</b> — নিচে পাঠানো হলো:`, { reply_markup: backKb });
            for (const w of list) {
                await tgSend(chatId,
        ...
        if (data === 'a_user') {
            ...
            await tgEdit(chatId, msgId, '👤 ইউজারের <b>Telegram numeric ID</b> পাঠান:', { reply_markup: cancelKb });
            return res.status(200).json({ ok: true });
        }
        ...
        if (data === 'a_flags') {
            ...
            if (!flagged.length) {
                await tgEdit(chatId, msgId, '✅ কোনো multi-account flag নেই।', { reply_markup: backKb });
                return res.status(200).json({ ok: true });
            }
            await tgEdit(chatId, msgId, `🚩 <b>${flagged.length} flagged account(s)</b> — নিচে পাঠানো হলো:`, { reply_markup: backKb });
            for (const u of flagged) {
                await tgSend(chatId,
                    `🚩 <b>Multi-Account Flag</b>\n\n` +
                    `👤 <code>${u._id}</code> (@${u.telegramUsername || '?'})\n` +
                    `🔗 একই ডিভাইস শেয়ার করছে: ${(u.multiAccountSiblings || []).map((s) => `<code>${s}</code>`).join(', ')}\n` +
                    `💰 Balance: ${u.wtcBalance || 0} WTC`,
                    { reply_markup: { inline_keyboard: [[
                        { text: '✅ Clear Flag (ঠিক আছে)', callback_data: `flagclear_${u._id}` },
                        { text: '🚫 Ban', callback_data: `ban_${u._id}` },
                    ]] } }
                );
            }
            return res.status(200).json({ ok: true });
        ...
        if (data === 'a_broadcast') {
            await setAdminState(fromId, { step: 'broadcast_msg' });
            await tgEdit(chatId, msgId, '📢 আপনার <b>broadcast message</b> টাইপ করুন:', { reply_markup: cancelKb });
            return res.status(200).json({ ok: true });
        }
    ...
    if (text.startsWith('/start')) {
        ...
        if (!ch || !com) {
            await tgSend(chatId,
                `👋 <b>Welcome to NEWTUBE!</b>\n\n⚠️ চালিয়ে যাওয়ার আগে আমাদের official channel ও community-তে join করুন।`,
                { reply_markup: { inline_keyboard: [
        ...
        await tgSend(chatId,
            `🎬 <b>Welcome to NEWTUBE!</b>\n\nভিডিও দেখুন · WTC আয় করুন · ক্রিপ্টো withdraw করুন!\n\n💰 TON ও USDT withdraw\n👥 ফ্রেন্ড রেফার করে বোনাস WTC`,
            { reply_markup: { inline_keyboard: [
                [{ text: '🚀 Open NEWTUBE', web_app: { url: APP_URL } }],
    ...
    if (s.step === 'user_lookup') {
        ...
        await tgSend(chatId,
            ...
            `📺 Ads Watched (lifetime): <b>${u.lifetimeAdsWatched || 0}</b>\n` +
    ...
    if (s.step === 'broadcast_msg') {
        ...
        let sent = 0, failed = 0;
        await tgSend(chatId, `📢 <b>${all.length}</b> ইউজারকে broadcast করা হচ্ছে...`);
        for (const u of all) {
            try { await tgSend(u._id, `📢 <b>NEWTUBE Update</b>\n\n${text}`); sent++; } catch { failed++; }
            await new Promise((r) => setTimeout(r, 50));
        }
        await tgSend(chatId, `✅ শেষ! Sent: <b>${sent}</b> | Failed: <b>${failed}</b>`, { reply_markup: adminKb });
        return res.status(200).json({ ok: true });
    }
    ...
    if (s.step === 'video_title') {
        await clearAdminState(fromId);
        await videos.insertOne({ videoId: s.videoId, title: text, isActive: true, createdAt: new Date() });
        await tgSend(chatId, `✅ <b>Video যুক্ত হয়েছে!</b>\n\n🎬 ${text}\n🔗 <code>${s.videoId}</code>`, { reply_markup: adminKb });
        return res.status(200).json({ ok: true });
    }
