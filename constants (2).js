...
// ⚠️ CHANGED (admin decision — per-network watch-time + reward tuning) —
...
export const AD_NETWORK_REWARDS = {
    adsgramDaily:   { reward: 10, dailyLimit: 10, minWatchSeconds: 10 },
    adsgramSpecial: { reward: 20, dailyLimit: 10, minWatchSeconds: 20 },
    monetag:        { reward: 8,  dailyLimit: 10, minWatchSeconds: 6 },
    giga:           { reward: 10, dailyLimit: 15, minWatchSeconds: 8, requiresClick: true },
    usl:            { reward: 15, dailyLimit: 10 },
...
// time).
// ⚠️ CHANGED — this is now only the FALLBACK for a network with no
...
// ⚠️ NEW (admin decision) — wallet lock. A user's FIRST-ever withdrawal
// method+address is saved as their permanent wallet (user.lockedWalletMethod
// / user.lockedWalletAddress) — every withdrawal after that reuses it
// automatically, no re-typing, and the client's submitted method/details
// are ignored/overridden by the locked value (protects against a tampered
// client trying to redirect a later payout to a different address).
//
// If the address turns out wrong, the admin's "Change Wallet Address" flow
// (wd_fixaddr_ / wd_new_address in api/bot.js) lets the user set a NEW
// permanent wallet — but only ONCE ever (user.walletChangeUsed). Using it
// closes out the CURRENT pending withdrawal as a refund: this percentage
// of that withdrawal's WTC is kept as a penalty, the rest goes back to
// their balance — then the new address becomes their locked wallet for
// future withdrawals. After this one-time change is used, any further
// wrong-address report falls back to the existing, more punitive
// wd_addrconfirm_ flow (WITHDRAW_SECOND_FEE... no relation — see that
// handler: refund minus a 50% penalty, wallet NOT updated).
...
//   1) WITHDRAW_FEE_PERCENT
//   2) WITHDRAW_SECOND_FEE_PERCENT (taken on what's left after fee 1)
// ⚠️ CHANGED (admin request) — was 25% + 5% (≈ 28.75% total). Now ONE flat
// 15% fee: fee 1 = 15, fee 2 = 0 (kept at 0 so the two-step maths in
// api/withdraw.js calcNetUsd() and index.html calcNetUsdDisplay() still work
// unchanged). A user now nets wtc/WTC_PER_USD * 0.85 = 85% of face value.
...
export const WITHDRAW_FEE_PERCENT = 15;        // ⚠️ CHANGED — was 25. Total withdraw fee is now 15%.
export const WITHDRAW_SECOND_FEE_PERCENT = 0;  // ⚠️ CHANGED — was 5. Set to 0: the extra fee was removed.
...
// ⚠️ CHANGED (big update) — was 72 hours, now 48 hours per admin request.
// A brand-new account can make its first withdrawal only after this many
// hours since joining. The withdraw modal (index.html), the status endpoint
// and the create endpoint (api/withdraw.js) all read from this one number.
export const WITHDRAW_MIN_ACCOUNT_AGE_HOURS = 48;

// ⚠️ NEW (big update) — promised payout window shown to users: once a
// withdrawal request is submitted the admin pays it somewhere between
// WITHDRAW_PROCESS_MIN_HOURS and WITHDRAW_PROCESS_MAX_HOURS. DISPLAY ONLY —
// nothing here auto-approves or auto-rejects; approval is still the admin's
// manual Approve/Reject flow in api/bot.js. Keep index.html's
// WITHDRAW_PROCESS_*_DISPLAY in sync with these two.
export const WITHDRAW_PROCESS_MIN_HOURS = 3;
export const WITHDRAW_PROCESS_MAX_HOURS = 48;

// ⚠️ NEW (big update) — Terms & Conditions gate. Every user must accept the
// current terms once (api/user.js action:'acceptTerms' → users.termsVersion)
// before earning or withdrawing (api/earn.js + api/withdraw.js reject with
// error:'terms_required' until then). Bump TERMS_VERSION whenever the terms
// text changes materially and everyone is asked to accept again.
// Keep index.html's TERMS_VERSION_DISPLAY in sync.
export const TERMS_VERSION = 1;
// true  = accounts that existed before this update must ALSO accept the terms
//         the next time they open the app (rules like the 48h wait and the
//         ban policy apply to everyone, so this is the default).
// false = only accounts created from now on are asked; older accounts are
//         marked as already accepted (grandfathered) on their next open.
export const TERMS_APPLY_TO_EXISTING_USERS = true;

// ⚠️ CHANGED (admin decision) — referral gate: the very first withdrawal a
// user ever makes is still free (no referral needed). Every withdrawal
// AFTER that now needs a NUMBER of valid referrals proportional to the USD
// amount being withdrawn — was a flat 1-per-withdrawal regardless of size.
// One valid referral covers every $0.25 (or part thereof) of the withdrawal:
// $0.25 needs 1, $0.26 needs 2, $0.50 needs 2, $0.51 needs 3, etc. — see
// requiredReferralsForUsd() below. A referral becomes "valid" once the
// referred user completes all 3 referral milestones (see lib/referral.js).
// Enforced in api/withdraw.js against user.validReferralCount -
// user.usedValidReferrals.
...
// ⚠️ CHANGED (admin decision) — price lowered 0.02→0.015 TON.
export const PUNCH_KEY_PRICE_NANOTON = 15_000_000; // 0.015 TON = 15,000,000 nanoTON
export const PUNCH_KEY_PRICE_TON = PUNCH_KEY_PRICE_NANOTON / 1_000_000_000; // 0.015 — display convenience
export const PUNCH_KEY_ORDER_EXPIRY_MINUTES = 15; // unpaid orders auto-expire this many minutes after creation
// ⚠️ NEW — lifetime cap on how many Punch Keys (valid-referral purchases) a
// single user can ever buy. Previously unlimited — an unlimited pay-to-skip
// on the referral gate undermines the whole point of that gate existing.
// Tracked via user.punchKeysPurchased (see api/payments.js's checkDeposits
// cron, where it's incremented alongside validReferralCount on each
// confirmed payment) — enforced in pkHandleCreate before a new order can
// even be created.
export const PUNCH_KEY_MAX_PURCHASES = 5;
// ══════════════════════════════════════════════════════════
...
// ⚠️ NEW — full date+time in the Bangladesh timezone, for admin-facing
...
// ⚠️ NEW — date-only counterpart to fmtBDDateTime, same timezone-correctness
...
// first" — see requiredReferralsForUsd() above.
...
// ⚠️ NEW (security update, after a real report) — extra spin limits, on top
...
export const SPIN_MIN_GAP_SECONDS = 60;
