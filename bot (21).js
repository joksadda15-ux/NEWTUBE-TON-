...
const BOT_TOKEN = process.env.BOT_TOKEN; // ⚠️ reused below as an internal shared-secret for self-continuation calls (see broadcast batching)
...
// ══════════════════════════════════════════════════════════
// ⚠️ FIX — BROADCAST NEVER ARRIVING
//
// Root cause: bc_confirm used to loop through EVERY user inline, one
// tgSend + 100ms delay at a time, and only returned the HTTP response
// after the ENTIRE loop finished. For 100+ users that's comfortably
// 15–30+ seconds of real wall-clock time (Telegram API latency included,
// not just the 100ms delay). Vercel serverless functions get killed at
// their configured maxDuration — and on the Hobby/free plan this is
// capped at 10s regardless of what vercel.json says (30s here is only
// honored on paid plans). So the function was being killed mid-loop:
// whatever users were reached before the kill silently got the message
// (if any), everyone after that got nothing, and the admin never saw the
// "✅ Done!" summary because that line never got a chance to run — which
// is exactly why it looked like "no one" received anything.
//
// Fix: broadcast is now a resumable BATCH JOB stored in Mongo
// (`broadcastJobs`). Each invocation only processes a small time-boxed
// slice (≤6s wall-clock, well under the 10s Hobby cap), then — if users
// remain — fires a fire-and-forget HTTP call to this same endpoint to
// process the next slice as a FRESH function invocation with a fresh
// timer. This works at any user count, not just today's ~100.
// ══════════════════════════════════════════════════════════
const BROADCAST_BATCH_TIME_BUDGET_MS = 6000; // stay well under the 10s Hobby-plan hard cap
const BROADCAST_SEND_DELAY_MS = 60;          // ~16 msg/sec, safely under Telegram's 30/sec bulk limit

async function processBroadcastBatch(db, jobId) {
    const jobs = db.collection('broadcastJobs');
    // jobId arrives as a real ObjectId on the inline first-batch call, but as
    // a plain string on the self-continuation call (JSON can't carry ObjectId).
    const _id = jobId instanceof ObjectId ? jobId : new ObjectId(String(jobId));
    const job = await jobs.findOne({ _id });
    if (!job) return; // already finished/cleaned up, or invalid id
    ...
    const remaining = job.remainingIds || [];
    let i = 0;

    while (i < remaining.length && Date.now() - startedAt < BROADCAST_BATCH_TIME_BUDGET_MS) {
        const uid = remaining[i];
        try {
            if (job.photoFileId) {
                await tgSendPhoto(uid, job.photoFileId, job.text, extra);
            } else {
                await tgSend(uid, job.text, extra);
            }
            sent++;
        } catch {
            failed++;
    ...
    if (stillRemaining.length === 0) {
        ...
        await tgSend(job.adminChatId, `✅ Broadcast done! Sent: <b>${sent}</b> | Failed: <b>${failed}</b>`, { reply_markup: adminKb });
        return;
    }

    await jobs.updateOne({ _id }, { $set: { remainingIds: stillRemaining, sent, failed, updatedAt: new Date() } });

    // ── fire-and-forget continuation — don't await the response, just get
    // the request on the wire so Vercel spins up a fresh invocation with a
    // fresh timer to process the next slice. Errors here are non-fatal:
    // worst case the job just sits until manually resumed (see a_broadcast_resume below).
    fetch(`${APP_URL}/api/bot`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ __broadcastContinue: true, jobId: String(jobId), secret: BOT_TOKEN }),
    }).catch(() => {});
}
...
export default async function handler(req, res) {
    ...
    // ── internal self-continuation call for the broadcast batch job — see
    // processBroadcastBatch above. Not a real Telegram update, so it's
    // handled here BEFORE any Telegram update parsing, gated by BOT_TOKEN
    // as a shared secret (never exposed to the client/frontend).
    if (req.body?.__broadcastContinue === true) {
        if (!BOT_TOKEN || req.body?.secret !== BOT_TOKEN) return res.status(401).json({ ok: false });
        const { db } = await connectToDatabase();
        await processBroadcastBatch(db, req.body.jobId);
    ...
    if (update.callback_query) {
        ...
        if (data.startsWith('wd_approve_') || data.startsWith('wd_reject_')) {
            ...
            // ⚠️ NEW — on approval, attach a button straight to the payment-proof
            // channel below the notification so the user can see their own
            // (and others') completed withdrawals posted there themselves.
            const notifExtra = approve
                ? { reply_markup: { inline_keyboard: [[{ text: '📢 View Payment Channel', url: `https://t.me/${PAYMENT_CHANNEL.replace('@', '')}` }]] } }
                : {};
            await tgSend(w.userId, notif, notifExtra);

            // ⚠️ NEW — this was the actual bug: NEWTUBE never posted approved
            // withdrawals to the public proof channel. Fruit Cut posts here on
            // every approval; NEWTUBE's approve handler simply had no equivalent
            // call. Mirrors Fruit Cut's "Withdrawal Completed" format, address masked.
            if (approve) {
                ...
                await tgSendPhoto(PAYMENT_CHANNEL, PAYMENT_PROOF_PHOTO, proofText).catch((e) => {
        ...
        if (data === 'bc_confirm') {
            ...
            await tgEdit(chatId, msgId, `📢 Broadcasting to <b>${allIds.length}</b> users... (this may take a bit for large lists — you'll get a "✅ Done!" message when it's finished)`);

            const jobs = db.collection('broadcastJobs');
            const jobResult = await jobs.insertOne({
                adminChatId: chatId,
                text: bs.text,
                buttonText: bs.buttonText || null,
                buttonUrl: bs.buttonUrl || null,
                photoFileId: bs.photoFileId || null,
                remainingIds: allIds,
                sent: 0,
                failed: 0,
                createdAt: new Date(),
            ...
            // Process the first batch inline (fast feedback), then it
            // self-continues in fresh invocations until all users are done.
            await processBroadcastBatch(db, jobResult.insertedId);
            return res.status(200).json({ ok: true });
        }
