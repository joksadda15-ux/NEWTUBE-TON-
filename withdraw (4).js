...
async function handleCreate(req, res, db) {
    ...
    const updateOps = { $inc: { usdtBalance: -tier.usd, [tierCountField]: 1, withdrawalCount: 1 } };
    if (levelResolution.isFinal) {
        updateOps.$inc.level6WithdrawCount = 1;
    } else {
        updateOps.$inc.withdrawsUsedAtLevel = 1;
    }
    // ⚠️ NEW — this is what was actually letting the same user fire two
    // withdrawals at once (double-tap / retry): nothing on this doc ever
    // blocked a second 'create' while a first was still pending. usdtBalance
    // alone doesn't catch it if the balance covers both. withdrawPending is
    // now part of the SAME atomic filter/update below, so a concurrent second
    // request reads the already-true flag and the gate fails for it. Cleared
    // back to false in api/bot.js on both approve and reject.
    updateOps.$set = { withdrawPending: true };
    if (!lockStatus) {
        updateOps.$set.addressLockedAt = new Date();
        updateOps.$set.lockedWithdrawMethod = method;
        updateOps.$set.lockedWithdrawAddress = details;
    }
    ...
    if (!gate) {
        // Distinguish "you already have one in flight" from the generic
        // balance/level race message — the frontend should tell the user
        // to wait for the pending one instead of just "try again".
    ...
    // ⚠️ FIX — this whole block was the actual cause of two separate reports:
    // (1) "full info dekhai na" — Tier / Total withdrawals so far / Total
    //     referrals / timestamp were dropped when this got rebuilt after the
    //     file truncation, only User/Amount/Method/Address were kept.
    // (2) "click korle kichu hoy na" — no reply_markup was ever attached, so
    //     there were literally no Approve/Reject buttons on this message.
    //     (The buttons DID exist on the separate "Withdrawals" admin-menu
    //     listing in api/bot.js, but clicking those crashed too — see the
    //     w.feeWtc fix in that file. Both had to be fixed together.)
    if (ADMIN_ID) {
        const adminText =
            `💸 <b>New Withdraw Request</b>\n\n` +
            `👤 User: <code>${id}</code>${verified.user.username ? ' (@' + verified.user.username + ')' : ''}\n` +
            `🎯 Tier: <b>${tier.id}</b>\n` +
            `💰 Amount: <b>$${tier.usd.toFixed(4)} ${methodConfig.currency}</b>\n` +
