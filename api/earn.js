...
// — same trust model as the ad networks above (we don't get independent
// proof the ad played, just that the client awaited the SDK's promise).
// Spends a bonus (referral) spin first if one's available, otherwise a
// daily free spin (of SPIN_DAILY_FREE); the reward itself is picked from
// SPIN_SEGMENTS by weight. Both pool-checks are separate atomic
...
async function handleSpin(req, res, db, userId) {
    ...
    let gate = await users.findOneAndUpdate(
        { _id: userId, isBanned: { $ne: true }, bonusSpinsAvailable: { $gt: 0 }, ...REWARD_ELIGIBLE_FILTER },
        { $inc: { bonusSpinsAvailable: -1 } },
        { returnDocument: 'after' }
    );
    const usedBonus = !!gate;
    if (!gate) {
        gate = await users.findOneAndUpdate(
            { _id: userId, isBanned: { $ne: true }, dailySpinsUsed: { $lt: SPIN_DAILY_FREE }, ...REWARD_ELIGIBLE_FILTER },
            { $inc: { dailySpinsUsed: 1 } },
            { returnDocument: 'after' }
        );
    }
    ...
    if (!gate) {
        ...
        return res.status(400).json({ ok: false, error: 'no_spins_left' });
    }
    ...
    return res.status(200).json({
        ...
        segmentIndex,
        usedBonus,
        spinsLeftToday: Math.max(0, SPIN_DAILY_FREE - (credited?.dailySpinsUsed ?? gate.dailySpinsUsed ?? SPIN_DAILY_FREE)),
