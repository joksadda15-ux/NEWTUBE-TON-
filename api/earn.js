...
import {
    ...
    AD_NETWORK_REWARDS, AD_MIN_WATCH_SECONDS, AD_COOLDOWN_SECONDS, TASK_MIN_WAIT_SECONDS, TASK_COOLDOWN_SECONDS,
    SPIN_FREE_COOLDOWN_HOURS, SPIN_SEGMENTS,
...
// ⚠️ NEW (this update) — separate signing namespace for the Spin Wheel's
// ad-gate token. Mirrors signAdStart/signLootboxStart exactly. See
// handleSpin below for why this was added: the free spin previously had
// ZERO server-side proof that its "watch an ad first" cost was actually
// paid — the frontend just called the ad SDK directly and then hit `spin`
// with nothing to check, so a script could call `spin` in a raw loop
// limited only by the old daily counter.
const signSpinStart = (userId, startTime) =>
    crypto.createHmac('sha256', SECRET).update(`spin:${userId}:${startTime}`).digest('hex');
...
async function handleClaimLootbox(req, res, db, userId) {
    ...
    const gate = await users.findOneAndUpdate(
        ...
        [
            { $set: {
                ...
                // ⚠️ NEW (this update) — tracks lifetime lootbox claims so we can
                // tell whether THIS claim is the user's first ever (see
                // referral-milestone hook below). Kept separate from
                // usedLootboxStarts (which is single-use-token replay
                // protection, reset daily) — this counter is lifetime and
                // never reset.
                lootboxClaimCount: { $add: [{ $ifNull: ['$lootboxClaimCount', 0] }, 1] },
            } },
            { $set: {
                pendingVideoWTC: 0,
    ...
    // ⚠️ NEW (this update) — awards the referrer's 4th milestone
    // (REFERRAL_REWARDS.step4_firstLootbox, lib/constants.js) the first time
    // THIS user ever successfully claims a lootbox. `gate` here is the
    // document from BEFORE this update (returnDocument: 'before'), so
    // !gate.lootboxClaimCount means this claim is their first ever.
    if (!gate.lootboxClaimCount) {
        await maybeAwardReferralMilestones(db, userId, { lootboxClaimed: true });
    }
...
const COUNTER_FIELD = {
    ...
    monetagPopup: 'monetagPopupCountToday', // ⚠️ NEW — Monetag Rewarded Popup format, separate from `monetag`
};
async function handleClaimAdReward(req, res, db, userId) {
    ...
    // ⚠️ NEW (this update) — monetag/monetagPopup are now credited EXCLUSIVELY
    // via Monetag's own server-to-server postback (handleMonetagPostback in api/earn.js),
    // not this client-driven claim path at all — see that file's header for
    // why. Blocked here so a stale cached frontend (from before this change
    // shipped) can't still credit WTC through the old, weaker signed-token
    // heuristic instead of real S2S confirmation.
    if (network === 'monetag' || network === 'monetagPopup') {
        return res.status(400).json({ ok: false, error: 'use_postback_only' });
    }
    ...
    // ⚠️ CHANGED — was the shared AD_MIN_WATCH_SECONDS floor for every
    // network. Now uses THIS network's own minWatchSeconds (see
    // AD_NETWORK_REWARDS in lib/constants.js), falling back to the shared
    // constant for any network that doesn't set one. Must have taken at
    // least that many seconds since the token was issued — a genuine ad SDK
    // flow always takes real wall-clock time (loading + showing the ad); an
    // instant claim right after adStart means no ad was actually shown.
    const minWatchSeconds = config.minWatchSeconds ?? AD_MIN_WATCH_SECONDS;
...
// ── spin ── ⚠️ NEW — Spin Wheel. Watching an ad (client picks randomly from
// usl/giga/adsgramDaily, per the admin's spec) is the "cost" of a spin turn.
// ⚠️ CHANGED — now backed by a real signed token (spinStart/signSpinStart),
// same as every other ad-gated reward, instead of pure client-side trust.
// Spends a bonus (referral) spin first if one's available, otherwise the
// free spin (subject to SPIN_FREE_COOLDOWN_HOURS); the reward itself is
// picked from SPIN_SEGMENTS by weight. Both pool-checks are separate atomic
// findOneAndUpdate calls so a double-tap can't double-spend either pool.
function rollSpinReward() {
    ...
    for (let i = 0; i < SPIN_SEGMENTS.length; i++) {
        ...
        if (roll <= 0) return { value: SPIN_SEGMENTS[i].value, segmentIndex: i };
    }
    return { value: SPIN_SEGMENTS[0].value, segmentIndex: 0 }; // float-rounding fallback, practically unreachable
}
// ── spinStart ── ⚠️ NEW (this update) — issues a short-lived signed token
// the instant the user taps "Watch ad & Spin", before the ad SDK is even
// called. Mirrors handleAdStart exactly — no DB write, no reward, just a
// signature the client must carry through the real ad flow and hand back
// to `spin`. See the security-fix note on SPIN_FREE_COOLDOWN_HOURS in
// lib/constants.js for why this needed adding.
async function handleSpinStart(req, res, db, userId) {
    const startTime = Date.now();
    return res.status(200).json({ ok: true, startTime, signature: signSpinStart(userId, startTime) });
}
async function handleSpin(req, res, db, userId) {
    ...
    if (signSpinStart(userId, startTime) !== signature) {
        return res.status(400).json({ ok: false, error: 'invalid_ad_token' });
    }
    const elapsedSeconds = (Date.now() - Number(startTime)) / 1000;
    if (isNaN(elapsedSeconds) || elapsedSeconds < 0) {
    ...
    const users = db.collection('users');
    await ensureDailyReset(users, userId);
    const spinStartKey = String(startTime); // single-use, same pattern as usedAdStarts/usedTaskStarts

    // Bonus (referral) spins are tried first, same priority as before —
    // they're a scarcer, earned resource so they're spent before the free
    // cooldown slot. NOT subject to SPIN_FREE_COOLDOWN_HOURS at all.
    let gate = await users.findOneAndUpdate(
        { _id: userId, isBanned: { $ne: true }, bonusSpinsAvailable: { $gt: 0 }, usedSpinStarts: { $ne: spinStartKey }, ...REWARD_ELIGIBLE_FILTER },
        { $inc: { bonusSpinsAvailable: -1 }, $addToSet: { usedSpinStarts: spinStartKey } },
        { returnDocument: 'after' }
    );
    const usedBonus = !!gate;
    if (!gate) {
        // ⚠️ CHANGED — was a daily counter (dailySpinsUsed < SPIN_DAILY_FREE),
        // now a rolling cooldown (lastFreeSpinAt vs SPIN_FREE_COOLDOWN_HOURS).
        const cooldownCutoff = new Date(Date.now() - SPIN_FREE_COOLDOWN_HOURS * 60 * 60 * 1000);
        gate = await users.findOneAndUpdate(
            {
                _id: userId, isBanned: { $ne: true }, usedSpinStarts: { $ne: spinStartKey },
                $or: [{ lastFreeSpinAt: { $exists: false } }, { lastFreeSpinAt: { $lte: cooldownCutoff } }],
                ...REWARD_ELIGIBLE_FILTER,
            },
            { $set: { lastFreeSpinAt: new Date() }, $addToSet: { usedSpinStarts: spinStartKey } },
            { returnDocument: 'after' }
        );
    }

    if (!gate) {
        const exists = await users.findOne({ _id: userId }, { projection: { isBanned: 1, multiAccountFlag: 1, channelVerified: 1, lastFreeSpinAt: 1, usedSpinStarts: 1 } });
        if (!exists) return res.status(404).json({ ok: false, error: 'user_not_found' });
        if (exists.isBanned) return res.status(403).json({ ok: false, error: 'banned' });
        if ((exists.usedSpinStarts || []).includes(spinStartKey)) {
            return res.status(400).json({ ok: false, error: 'ad_token_already_used' });
        }
        if (exists.multiAccountFlag && !exists.channelVerified) return res.status(403).json({ ok: false, error: 'account_under_review' });
        const retryAfterMs = exists.lastFreeSpinAt
            ? Math.max(0, new Date(exists.lastFreeSpinAt).getTime() + SPIN_FREE_COOLDOWN_HOURS * 60 * 60 * 1000 - Date.now())
            : 0;
        return res.status(400).json({ ok: false, error: 'no_spins_left', retryAfterMs });
    }

    const { value: reward, segmentIndex } = rollSpinReward();
    ...
    return res.status(200).json({
        ...
        segmentIndex,
        usedBonus,
        // ⚠️ CHANGED — replaces spinsLeftToday. Frontend computes the
        // countdown as lastFreeSpinAt + SPIN_FREE_COOLDOWN_HOURS.
        lastFreeSpinAt: credited?.lastFreeSpinAt ?? gate.lastFreeSpinAt ?? null,
        bonusSpinsLeft: credited?.bonusSpinsAvailable ?? gate.bonusSpinsAvailable ?? 0,
    });
}
...
async function handleTaskComplete(req, res, db, userId) {
    ...
    if (isApiVerified) {
        ...
        // ⚠️ NEW — cross-task cooldown (see TASK_COOLDOWN_SECONDS in
        // lib/constants.js). A quick pre-check here just for a clean, fast
        // rejection before wasting a task "slot" claim below — the REAL
        // enforcement (closing the race window) is the atomic filter on
        // lastTaskClaimAt in the STEP 2 update further down.
        if (user.lastTaskClaimAt && new Date(user.lastTaskClaimAt).getTime() > Date.now() - TASK_COOLDOWN_SECONDS * 1000) {
            return res.status(400).json({
                ok: false, error: 'task_cooldown',
                retryAfterMs: new Date(user.lastTaskClaimAt).getTime() + TASK_COOLDOWN_SECONDS * 1000 - Date.now(),
            });
        }
    ...
    const gate = await users.findOneAndUpdate(
        {
            ...
            ...(taskStartKey ? {
                usedTaskStarts: { $ne: taskStartKey },
                // ⚠️ NEW — atomic re-check of the cooldown pre-check above, closing
                // the race window (e.g. two rapid double-taps landing near the
                // boundary, or two different tasks' claims arriving milliseconds
                // apart). Only applies to non-channel tasks (taskStartKey set).
                $or: [{ lastTaskClaimAt: { $exists: false } }, { lastTaskClaimAt: { $lte: new Date(Date.now() - TASK_COOLDOWN_SECONDS * 1000) } }],
            } : {}),
            ...REWARD_ELIGIBLE_FILTER,
        },
        {
            ...
            // ⚠️ NEW — only stamped for non-channel tasks; channel/api-verified
            // tasks aren't subject to the cooldown so there's no need to track it
            // for them (and no reason to make an unrelated 'channel' claim wait on
            // an earlier non-channel task's timer, or vice versa).
            ...(taskStartKey ? { $set: { lastTaskClaimAt: new Date() } } : {}),
        },
        { returnDocument: 'after' }
    );
    if (!gate) {
        ...
        const exists = await users.findOne({ _id: userId }, { projection: { completedTasks: 1, multiAccountFlag: 1, channelVerified: 1, usedTaskStarts: 1, lastTaskClaimAt: 1 } });
        if (taskStartKey && (exists?.usedTaskStarts || []).includes(taskStartKey)) {
            return res.status(400).json({ ok: false, error: 'task_token_already_used' });
        }
        // ⚠️ NEW — surface the cooldown specifically (instead of falling through
        // to the generic alreadyDone below) if that's what actually blocked it.
        if (taskStartKey && exists?.lastTaskClaimAt && new Date(exists.lastTaskClaimAt).getTime() > Date.now() - TASK_COOLDOWN_SECONDS * 1000) {
            return res.status(400).json({
                ok: false, error: 'task_cooldown',
                retryAfterMs: new Date(exists.lastTaskClaimAt).getTime() + TASK_COOLDOWN_SECONDS * 1000 - Date.now(),
            });
        }
    ...
    // ⚠️ NEW — user-created tasks (see api/payments.js (resource:'taskcreate')) auto-delete
    // USER_TASK_COMPLETED_TTL_DAYS after they hit their completion limit
    // (TTL index in models/schema.js). Only fires for isUserCreated:true
    // tasks — an admin-set limit on a normal task is never auto-deleted
    // this way. Placed AFTER the reward-credit gate above succeeds (not
    // right after taskGate) so a completion that gets rolled back (STEP 2
    // failing, slot given back at line ~597) never wrongly marks the task
    // "completed" while its real completionCount is actually back below limit.
    if (taskGate.isUserCreated && taskGate.limit > 0 && taskGate.completionCount >= taskGate.limit) {
        tasks.updateOne({ _id: taskObjId, completedAt: { $exists: false } }, { $set: { completedAt: new Date() } }).catch(() => {});
    }
...
// ── Monetag S2S postback ── ⚠️ NEW (this update) — merged directly into
// earn.js instead of its own api/monetag-postback.js file, because this
// project is already at Vercel's Hobby-plan 12-serverless-function limit
// (11 files under api/ + 2 nested under api/admin, api/cron = 12) — adding
// a 13th file breaks deployment entirely. Routed via a GET request with
// `?pb=monetag` (see the very top of the default export below), since
// everything else in this file is POST-only and requires a verified
// Telegram session, which a server-to-server postback has neither of.
//
// Real server-to-server verification for Monetag, per admin request
// ("must click", "top notch security"). Monetag's own Telegram Mini App
// SDK supports genuine S2S postbacks for exactly the two formats we use
// (Rewarded Interstitial = `monetag`, Rewarded Popup = `monetagPopup`) —
// see https://docs.monetag.com/docs/postbacks/. Unlike every other ad
// network in this app (Adsgram, GigaPub, USL — where "watched the ad" is
// still just a signed-token + timing heuristic), Monetag's OWN backend hits
// THIS endpoint directly once it has independently confirmed the event.
// The client's `show_9442539()` promise resolving no longer credits WTC
// for these two networks at all — only this postback does.
//
// ── Admin setup step (one-time, in Monetag's dashboard) ──
// 1. publishers.monetag.com → Telegram Mini Apps → NewTube12_bot → Postback
// 2. "Your backend URL" field, paste (replace YOUR_DOMAIN and YOUR_SECRET):
//      https://YOUR_DOMAIN/api/earn?pb=monetag&secret=YOUR_SECRET&ymid={ymid}&event={event_type}&reward_type={reward_event_type}&telegram_id={telegram_id}
// 3. Set the SAME secret as the MONETAG_POSTBACK_SECRET environment
//    variable on Vercel — this is a plain shared secret WE add to the URL
//    (Monetag's postback isn't cryptographically signed), so keep it
//    private exactly like VIDEO_SIGNING_SECRET — never in index.html.
// 4. Enable postbacks for BOTH Rewarded Interstitial and Rewarded Popup.
//
// ── ⚠️ RESOLVED — telegram_id confirmed ──
// The full official docs (docs.monetag.com/docs/postbacks/macroses/) confirm
// {telegram_id} is "Telegram user ID from Telegram WebApp SDK (auto-collected,
// may be empty)" — a real per-user ID, NOT the App ID. The dashboard's own
// "App ID, in Int format" label for this macro was just a UI text bug on
// Monetag's side. The ymid-vs-telegram_id cross-check below is correct as designed.
//
// ── ⚠️ UNRESOLVED — reward_event_type: two of Monetag's own sources disagree ──
// The official docs (Macro Reference AND the Postback Troubleshooting FAQ)
// both say this field is "valued" (monetized) / "not_valued" (shown but
// filtered, e.g. fraud). But the admin's own NewTube12_bot postback
// dashboard screen literally says: "Reward event type — String 'yes' if the
// event is paid, or 'no'." Rather than bet on one Monetag source over the
// other, the check below accepts EITHER 'valued' OR 'yes' as "this event
// was actually monetized" — whichever format Monetag's servers really send,
// a genuine paid event passes and a genuine unpaid one ('not_valued'/'no')
// is still rejected either way.
//
// ── Why `event=click` is required ──
// An impression-only postback (ad shown but never engaged) does NOT credit
// any WTC — this is the literal implementation of "must click".
async function handleMonetagPostback(req, res, db) {
    const { secret, ymid, event, reward_type, telegram_id } = req.query;
    const POSTBACK_SECRET = process.env.MONETAG_POSTBACK_SECRET;

    if (!POSTBACK_SECRET) {
        console.error('MONETAG_POSTBACK_SECRET is not set — rejecting all Monetag postbacks until it is.');
        return res.status(200).json({ ok: false, error: 'server_misconfigured' });
    }
    // ⚠️ Plain string comparison is fine here — this is a shared secret in
    // a query param, not a cryptographic signature to defend against timing
    // attacks; the real defense is keeping this URL private.
    ...
    // ── decode ymid → { userId, network } ──
    // ymid is generated client-side in index.html as `${userId}:${network}:${nonce}`
    // when the ad is started (see showMonetagAd/showMonetagPopupAd). It isn't a
    // security boundary by itself (see the telegram_id cross-check below) — it's
    // just how we route this postback to the right user/network/reward amount.
    const parts = String(ymid).split(':');
    if (parts.length !== 3) return res.status(200).json({ ok: false, error: 'invalid_ymid' });
    const [ymidUserId, network] = parts;

    // ⚠️ Cross-check — the userId encoded in ymid must match {telegram_id}.
    // See the "IMPORTANT" note above the header comment if this macro turns
    // out not to be what it's assumed to be.
    if (String(telegram_id) !== String(ymidUserId)) {
        return res.status(200).json({ ok: false, error: 'telegram_id_mismatch' });
    }

    const config = AD_NETWORK_REWARDS[network];
    if (!config || (network !== 'monetag' && network !== 'monetagPopup')) {
        return res.status(200).json({ ok: false, error: 'invalid_network' });
    }

    // ⚠️ "Must click" — an impression-only postback never credits WTC.
    if (event !== 'click') {
        return res.status(200).json({ ok: true, ignored: true, reason: 'not_a_click_event' });
    }
    // ⚠️ Only reward events Monetag itself actually monetized — accepts
    // EITHER 'valued' (official docs) or 'yes' (this account's own
    // dashboard text) since Monetag's two sources disagree; see the
    // "UNRESOLVED" note above this function.
    if (reward_type && reward_type !== 'valued' && reward_type !== 'yes') {
        return res.status(200).json({ ok: true, ignored: true, reason: 'not_valued' });
    }

    const users = db.collection('users');

    // ── idempotency — Monetag retries postbacks on anything but a 200, and
    // could plausibly redeliver the same event more than once regardless.
    // A dedicated collection with `_id: ymid` and a first-write-wins insert
    // guarantees this exact event is only ever credited once. ──
    const processed = db.collection('monetagProcessedYmids');
    try {
        await processed.insertOne({ _id: ymid, userId: ymidUserId, network, processedAt: new Date() });
    } catch (err) {
        if (err.code === 11000) return res.status(200).json({ ok: true, alreadyProcessed: true });
        throw err;
    }

    const counterField = network === 'monetag' ? 'monetagCountToday' : 'monetagPopupCountToday';
    const reward = config.reward;

    // ⚠️ Daily limit is still enforced here, atomically, same as every other
    // network's claimAdReward — a confirmed real click still shouldn't be
    // payable past the network's own daily cap.
    const gate = await users.findOneAndUpdate(
        { _id: ymidUserId, isBanned: { $ne: true }, [counterField]: { $lt: config.dailyLimit } },
        { $inc: { wtcBalance: reward, lifetimeWtcEarned: reward, [counterField]: 1 } },
        { returnDocument: 'after' }
    );

    if (!gate) {
        return res.status(200).json({ ok: true, credited: false, reason: 'gate_failed' });
    }
    return res.status(200).json({ ok: true, credited: true, reward, network, userId: ymidUserId });
}

export default async function handler(req, res) {
    // ⚠️ NEW (this update) — Monetag's S2S postback arrives as a GET with no
    // Telegram session at all, so it's routed BEFORE the POST-only check and
    // BEFORE initData verification just below. See handleMonetagPostback's
    // own header comment for the full picture (setup steps, security model).
    if (req.method === 'GET' && req.query.pb === 'monetag') {
        const { db } = await connectToDatabase();
        return handleMonetagPostback(req, res, db);
    }
    ...
    switch (action) {
        ...
        case 'spinStart':       return handleSpinStart(req, res, db, userId);
