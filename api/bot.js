...
import { WEEKLY_REFERRAL_MIN_COUNT, WEEKLY_REFERRAL_MAX_WINNERS, WTC_PER_USD, WITHDRAW_REFERRAL_COMMISSION_PERCENT, fmtBDDateTime } from '../lib/constants.js';
...
async function renderPendingReviewList(tasksCol) {
    ...
    if (!pending.length) {
        return ['✅ কোনো Task Review-এর অপেক্ষায় নেই।', backKb];
    }
    let text_ = `🕐 <b>Pending Review</b> (${pending.length})\n\n`;
    const rows = [];
    pending.forEach(t => {
        text_ += `📋 <b>${t.title}</b>\n🔗 ${t.url}\n🔎 Type: ${t.verifyType === 'api' ? 'Channel/Group Join (API)' : 'Link'} · 👥 ${t.limit} slots · 💰 ${t.rewardWtc} WTC/জন\n🧾 Creator ID: <code>${t.createdBy}</code>\n\n`;
        rows.push([
