...
export default async function handler(req, res) {
    ...
    if (update.callback_query) {
        ...
        if (data === 'bc_confirm') {
            ...
            // ⚠️ BUG FIX #1 (this update) — this used to be a bare,
            // un-awaited `fetch(...)`. Vercel freezes the function's
            // execution the instant the response below is sent, killing
            // any in-flight network call that hasn't finished — so this
            // trigger never actually went out, the worker chain never
            // started, and NO ONE ever received the broadcast (confirmed:
            // only the "queued" message above ever arrived, nothing else).
            //
            // ⚠️ BUG FIX #2 (this update) — waitUntil()'s promise still has
            // to settle within THIS invocation's own maxDuration deadline.
            // Waiting for broadcastWorker's full response means waiting for
            // its entire first chunk (~6-8s) to finish processing — cutting
            // it close against Hobby's 10s cap on top of whatever this
            // handler already spent. We only need the request to be sent,
            // not answered, so we abort our own wait after 3s; the worker
            // invocation keeps running independently once triggered,
            // regardless of whether we're still listening for its response.
            ...
            waitUntil((() => {
                ...
                return fetch(`${APP_URL}/api/broadcastWorker?jobId=${job._id}&secret=${BOT_TOKEN_FOR_WORKER}`, { signal: controller.signal })
                    .catch(() => {}) // expected: AbortError once the 3s cutoff hits
