    ...
    if (update.callback_query) {
        ...
        if (data === 'a_flags') {
            ...
            for (const u of flagged) {
                ...
                groupText += `\n⚠️ All accounts in this group will be <b>banned automatically</b>.\nIf a user contacts you claiming it was a mistake, you can unban manually.`;

                // Auto-ban all accounts in the group immediately
                await users.updateMany({ _id: { $in: allInGroup } }, { $set: { isBanned: true, multiAccountFlag: false } });

                // Notify each banned user
                for (const id of allInGroup) {
                    await tgSend(id,
                        `⛔ <b>Account Banned</b>\n\nYour account was banned for using multiple accounts on the same device.\n\nIf you believe this is a mistake, please contact the admin.`
                    ).catch(() => {});
                }

                const rows = [[{ text: '✅ Unban All (mistake)', callback_data: `devban_clearall_${allInGroup.join(',')}` }]];
                await tgSend(chatId, groupText + `\n\n✅ <b>Auto-banned ${allInGroup.length} account(s).</b>`, { reply_markup: { inline_keyboard: rows } });
            }
            return res.status(200).json({ ok: true });
        }

        // ── Device bulk-ban: keep one, ban the rest ──
        // ── Device unban all (admin decides it was a mistake) ──
