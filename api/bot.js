...
export default async function handler(req, res) {
    ...
    if (update.callback_query) {
        ...
        if (data === 'bc_confirm') {
            ...
            // ⚠️ BUG FIX (this update) — this used to be a bare, un-awaited
            // `fetch(...)`. Vercel freezes the function's execution the
            // instant the response below is sent, killing any in-flight
            // network call that hasn't finished — so this trigger never
            // actually went out, the worker chain never started, and NO
            // ONE ever received the broadcast (confirmed: only the
            // "queued" message above ever arrived, nothing else).
            // waitUntil() tells Vercel to keep the function alive until
            // this promise settles, even after the response is sent.
