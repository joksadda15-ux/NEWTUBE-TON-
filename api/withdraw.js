...
//   1) WITHDRAW_FEE_PERCENT (now 15% — was 25%; total fee is 15%, see lib/constants.js)
//   2) WITHDRAW_SECOND_FEE_PERCENT (now 0% — the extra 5% was removed)
// Net received = 85% of face value (0.85 × 1.00). See calcNetUsd().
//
...
async function handleStatus(req, res, db) {
    ...
    return res.status(200).json({
        ...
        // ⚠️ NEW (big update) — promised payout window (display only), and
        // whether this user has accepted the current Terms & Conditions.
        ...
        // null means the user hasn't withdrawn yet — the frontend shows the
        // normal method+address form and it gets locked on first submit.
...
async function handleCreate(req, res, db) {
    ...
    // ⚠️ NEW (big update) — must have accepted the Terms & Conditions first.
    if (user.termsVersion !== TERMS_VERSION) return res.status(403).json({ ok: false, error: 'terms_required' });

    // ── wallet lock ── ⚠️ NEW (see WALLET_CHANGE_PENALTY_PERCENT, lib/constants.js)
    const hasLockedWallet = !!(user.lockedWalletMethod && user.lockedWalletAddress);
    if (hasLockedWallet) {
        // Ignore whatever the client sent — always use the locked wallet.
        // This also protects against a tampered/stale client trying to
        // redirect a later payout to a different address than the one
        // that's actually locked in.
        method = user.lockedWalletMethod;
        details = user.lockedWalletAddress;
    } else if (!method || !details) {
        // First-ever withdrawal — method+details are required from the
        // client this one time; they get locked in below once this
        // request succeeds.
    ...
    // ⚠️ NEW — locks the wallet in on the very first successful withdrawal.
    if (!hasLockedWallet) {
    ...
    const withdrawDoc = {
        ...
        // ⚠️ REMOVED — addressFixUsed (was per-withdrawal). The wallet-change
        // allowance is now tracked per-USER, lifetime (user.walletChangeUsed,
        // see WALLET_CHANGE_PENALTY_PERCENT in lib/constants.js and
        // wd_fixaddr_ in api/bot.js), not on the withdrawal doc.
    };
    ...
    if (ADMIN_ID) {
        const adminText =
            ...
            // ⚠️ FIX — was `withdrawDoc.createdAt.toLocaleString()` with no
            // timezone, which formats in the SERVER's timezone (Vercel =
            // UTC), not Bangladesh time — this was the ACTUAL source of the
            // "wrong time" the admin kept seeing on every new withdraw
            // request (a previous fix touched api/bot.js's approve/reject/
            // flag timestamps, but missed this one, the very first message
