...
//   7) A rejected withdraw fully refunds dogsAmount back into dogsBalance
//
// ⚠️ THIS PASS — 3 bug fixes, all from the DOGS/Season-3 currency rebuild
// (api/withdraw.js, lib/constants.js) not having been mirrored here yet:
//   A) "🚫 Keep Suspended" pointed at the shared `noop_dismiss` callback,
//      which has no handler anywhere in this file — clicking it did
//      nothing. It now has its own `keepban_<id>` handler.
//   B) wd_approve/wd_reject and the a_pending list still referenced the old
//      USD-tier withdrawal fields (`cashAmount`, `tierId`, `wtcAmount`,
//      `feeWtc`) and refunded rejections into `usdtBalance` — none of which
//      exist anymore now that withdrawals store `dogsAmount` and users have
//      `dogsBalance`. This is almost certainly why "Withdrawals" looked
//      broken/empty and why rejected withdrawals weren't actually being
//      refunded. Everything now reads/writes `dogsAmount` / `dogsBalance`.
//   C) renderUserLookup's lifetime-withdrawn stat summed `w.wtcAmount`
//      (always 0 now) — fixed to sum `w.dogsAmount`.
// See the bottom of this file's chat response for why problem 3 (no
// withdrawals arriving at all) needs one more piece of info to confirm.
...
const adminKb = {
    inline_keyboard: [
        ...
        [{ text: '📰 Add Article', callback_data: 'a_addarticle' }, { text: '🚰 Add Faucet', callback_data: 'a_addfaucet' }],
        [{ text: '🗑 Manage Tasks', callback_data: 'a_managetasks_0' }, { text: '🗑 Manage Videos', callback_data: 'a_managevideos_0' }],
        [{ text: '🗑 Manage Articles', callback_data: 'a_managecat_article_0' }, { text: '🗑 Manage Faucets', callback_data: 'a_managecat_faucet_0' }],
...
function renderUserLookup(u, withdrawalsList = []) {
    ...
    const totalWithdrawnDogs = withdrawalsList.filter(w => w.status === 'approved').reduce((sum, w) => sum + (w.dogsAmount || 0), 0); // ⚠️ FIXED — was w.wtcAmount, a field that no longer exists on DOGS-schema withdrawals (always summed to 0)
    ...
    const text =
        ...
        `📤 Withdrawals: <b>${wCount}</b> (${pendingWithdrawCount} pending) — <b>${totalWithdrawnDogs.toLocaleString()} DOGS</b> approved lifetime\n` +
    ...
    const options = { reply_markup: { inline_keyboard: [
        ...
        [{ text: u.multiAccountFlag ? '🚩 Unflag Multi-Acc' : '🚩 Flag Multi-Acc', callback_data: `toggleflag_${u._id}` }],
...
export default async function handler(req, res) {
    ...
    if (update.callback_query) {
        ...
        if (data.startsWith('wd_approve_') || data.startsWith('wd_reject_')) {
            ...
            if (!approve) {
                // ⚠️ FIXED — this was still refunding into `usdtBalance` using
                // `w.cashAmount` and adjusting `withdrawTierCounts`, all of
                // which belonged to the old USD-tier system and no longer
                // exist (withdrawals now store `dogsAmount`, users now have
                // `dogsBalance`, and there are no more tiers). Because of
                // this mismatch, a rejected withdrawal was refunding into a
                // dead field — `w.cashAmount` was `undefined`, so
                // `$inc: { usdtBalance: undefined }` silently did nothing,
                // meaning the user's DOGS never actually came back to them.
                const refundUpdate = { $inc: { dogsBalance: w.dogsAmount, withdrawalCount: -1 }, $set: { lastWithdrawDate: '' } };
            ...
            const notif = approve
                ? `🎉 <b>Congratulations!</b>\n\n` +
                  `You've received <b>${w.dogsAmount.toLocaleString()} DOGS</b>\n` +
                  `📍 <code>${w.details}</code>\n\n` +
                  `💪 Keep up the great work! Watch more ads, complete tasks, and refer your friends to earn even more WTC every day. 🚀`
                : `❌ <b>Withdrawal Rejected.</b>\nYour ${w.dogsAmount.toLocaleString()} DOGS has been refunded to your converted balance.`;
            ...
            const processedText =
                ...
                `💰 <b>${w.dogsAmount.toLocaleString()} DOGS</b>\n` +
        ...
        // ── Keep Suspended (appeal reviewed, admin declines to unban) ──
        // ⚠️ NEW — the "🚫 Keep Suspended" button used to point at the shared
        // `noop_dismiss` callback_data, which has no handler anywhere in this
        // file — so clicking it did nothing at all (no DB change, no message
        // update, not even the loading spinner clearing properly). It now has
        // its own callback and actually closes out the appeal.
        if (data.startsWith('keepban_')) {
            const target = data.replace('keepban_', '');
            await tgAnswerCallback(cb.id, 'Kept suspended', true);
            await tgEdit(chatId, msgId, `🚫 <b>Kept Suspended</b> — <code>${target}</code> stays banned. Appeal dismissed.`, { reply_markup: backKb });
            return res.status(200).json({ ok: true });
        }

        // ── Decorative/label-only buttons (e.g. the video on/off row in
        // Manage Videos) — not actionable, but still answered so the button
        // doesn't sit there spinning. ──
        if (data === 'noop_dismiss') {
            await tgAnswerCallback(cb.id);
            return res.status(200).json({ ok: true });
        }
        ...
        if (data.startsWith('zerobal_')) {
            ...
            const u = await users.findOne({ _id: target });
            if (!u) { await tgEdit(chatId, msgId, '❌ User not found.', { reply_markup: backKb }); return res.status(200).json({ ok: true }); }
            const seizedWtc = u.wtcBalance || 0;
            const seizedDogs = u.dogsBalance || 0; // ⚠️ NEW — also zero the already-converted, ready-to-withdraw DOGS balance; previously only wtcBalance was seized, so a confiscated user could still cash out anything they'd already converted
            await users.updateOne({ _id: target }, { $set: { wtcBalance: 0, dogsBalance: 0 } });
            await tgEdit(chatId, msgId, `💸 Confiscated <b>${seizedWtc.toLocaleString()} WTC</b> + <b>${seizedDogs.toLocaleString()} DOGS</b> from <code>${target}</code> — both balances are now 0.`, { reply_markup: { inline_keyboard: [[{ text: '👤 View User', callback_data: `lookup_${target}` }], [{ text: '◀️ Back to Menu', callback_data: 'a_menu' }]] } });
            return res.status(200).json({ ok: true });
        }
        ...
        // ── Manually toggle the multi-account flag (for cases the automated fingerprint
        // check missed, or to clear a false positive) ──
        if (data.startsWith('toggleflag_')) {
            const target = data.replace('toggleflag_', '');
            const u = await users.findOne({ _id: target }, { projection: { multiAccountFlag: 1 } });
            if (!u) { await tgEdit(chatId, msgId, '❌ User not found.', { reply_markup: backKb }); return res.status(200).json({ ok: true }); }
            const newFlag = !u.multiAccountFlag;
            await users.updateOne({ _id: target }, { $set: { multiAccountFlag: newFlag } });
            await tgEdit(chatId, msgId, `🚩 <code>${target}</code> is now ${newFlag ? '<b>flagged</b>' : '<b>unflagged</b>'}.`, { reply_markup: { inline_keyboard: [[{ text: '👤 View User', callback_data: `lookup_${target}` }], [{ text: '◀️ Back to Menu', callback_data: 'a_menu' }]] } });
            return res.status(200).json({ ok: true });
        }
        ...
        if (data === 'task_cat_channel' || data === 'task_cat_partner' || data === 'task_cat_faucet' || data === 'task_cat_article') {
            ...
            if (data === 'task_cat_channel') {
                ...
                s.category = data === 'task_cat_faucet' ? 'faucet' : data === 'task_cat_article' ? 'article' : 'partner';
                s.step = 'task_url';
                await setAdminState(fromId, s);
                const prompt = s.category === 'faucet' ? "Now send the faucet site's <b>referral link</b>:"
                    : s.category === 'article' ? "Now send the <b>article link</b> (Blogger post URL):"
                    : "Now send the task's <b>link</b> (YouTube/FB/Bot, etc.):";
        ...
        if (data === 'a_flags') {
            ...
            for (const u of flagged) {
                ...
                await users.updateOne({ _id: mainAcc._id }, { $set: { isBanned: false, multiAccountFlag: false }, $unset: { bannedAt: '' } });
                await markUnbanned(db, [mainAcc._id]);
                if (others.length) {
                    await users.updateMany({ _id: { $in: others.map(o => o._id) } }, { $set: { isBanned: true, multiAccountFlag: false, bannedAt: new Date() } });
                    await markBanned(db, others.map(o => o._id));
                }
        ...
        if (data.startsWith('devmain_')) {
            ...
            await users.updateOne({ _id: targetId }, { $set: { isBanned: false, multiAccountFlag: false }, $unset: { bannedAt: '' } });
            await markUnbanned(db, [targetId]);
            if (rest.length) {
                await users.updateMany({ _id: { $in: rest } }, { $set: { isBanned: true, multiAccountFlag: false, bannedAt: new Date() } });
                await markBanned(db, rest);
            }
        ...
        if (data.startsWith('devban_clearall_')) {
            ...
            await users.updateMany({ _id: { $in: allIds } }, { $set: { isBanned: false, multiAccountFlag: false }, $unset: { bannedAt: '' } });
            await markUnbanned(db, allIds);
        ...
        // ── Shortcuts that skip the category-choice step (jumps straight to
        // title → URL → reward → limit → confirm, same underlying task system) ──
        if (data === 'a_addarticle') {
            await setAdminState(fromId, { step: 'task_title', presetCategory: 'article' });
            await tgEdit(chatId, msgId, `📰 <b>Add Article — Step 1/4</b>\n\nSend the article's <b>title</b>:`, { reply_markup: cancelKb });
            return res.status(200).json({ ok: true });
        }
        if (data === 'a_addfaucet') {
            await setAdminState(fromId, { step: 'task_title', presetCategory: 'faucet' });
            await tgEdit(chatId, msgId, `🚰 <b>Add Faucet — Step 1/4</b>\n\nSend the faucet site's <b>title</b>:`, { reply_markup: cancelKb });
            return res.status(200).json({ ok: true });
        }
        ...
        // ── 🗑 Manage Articles / Manage Faucets — same as Manage Tasks, filtered by category ──
        if (data.startsWith('a_managecat_')) {
            const rest = data.replace('a_managecat_', '');
            const [category, pageStr] = rest.split('_');
            const page = parseInt(pageStr) || 0;
            const perPage = 8;
            const filter = { category };
            const all = await tasks.find(filter).sort({ createdAt: -1 }).skip(page * perPage).limit(perPage).toArray();
            const totalCount = await tasks.countDocuments(filter);
            const label = category === 'faucet' ? '🚰 Faucets' : '📰 Articles';
            if (!all.length) {
                await tgEdit(chatId, msgId, page === 0 ? `📭 No ${label.toLowerCase()} yet.` : `📭 No more ${label.toLowerCase()}.`, { reply_markup: backKb });
                return res.status(200).json({ ok: true });
            }
            let text_ = `${label} <b>(${totalCount} total)</b>\n\n`;
            all.forEach(t => {
            ...
            const navRow = [];
            if (page > 0) navRow.push({ text: '◀️ Prev', callback_data: `a_managecat_${category}_${page - 1}` });
            if ((page + 1) * perPage < totalCount) navRow.push({ text: 'Next ▶️', callback_data: `a_managecat_${category}_${page + 1}` });
            if (navRow.length) rows.push(navRow);
            rows.push([{ text: '◀️ Back to Menu', callback_data: 'a_menu' }]);
                ...
                if (ADMIN_ID) {
                    await tgSend(ADMIN_ID,
                        ...
                        { reply_markup: { inline_keyboard: [[
                            ...
                            { text: '🚫 Keep Suspended', callback_data: `keepban_${targetId}` }, // ⚠️ FIXED — was `noop_dismiss`, a dead callback with no handler, so this button did nothing when clicked
                        ]] } }
                    ).catch(() => {});
                }
                return res.status(200).json({ ok: true });
    ...
    if (s.step === 'user_lookup') {
        ...
        if (!/^\d+$/.test(text)) {
            ...
            matches.forEach((u, i) => {
                out += `<b>${i + 1}.</b> ${u.firstName || 'User'} (@${u.telegramUsername || 'n/a'})\n   ID: <code>${u._id}</code> | 💰 ${u.wtcBalance || 0} WTC | 👥 ${u.referralCount || 0} refs | 📤 ${u.withdrawalCount || 0} withdraws${u.multiAccountFlag ? ' 🚩' : ''}${u.isBanned ? ' 🔒' : ''}\n\n`;
            });
            const rows = matches.map(u => [{ text: `👤 ${u.firstName || 'User'} (@${u.telegramUsername || 'n/a'})${u.multiAccountFlag ? ' 🚩' : ''}`, callback_data: `lookup_${u._id}` }]);
            rows.push([{ text: '◀️ Back to Menu', callback_data: 'a_menu' }]);
    ...
    if (s.step === 'task_title') {
        s.title = text;
        if (s.presetCategory) {
            s.category = s.presetCategory;
            s.step = 'task_url';
            await setAdminState(fromId, s);
            const prompt = s.category === 'faucet' ? "Now send the faucet site's <b>referral link</b>:" : "Now send the <b>article link</b> (Blogger post URL):";
            await tgSend(chatId, `📋 Title: ✅ <b>${s.title}</b>\n\n${prompt}`, { reply_markup: cancelKb });
            return res.status(200).json({ ok: true });
        }
        ...
        await tgSend(chatId, `📋 <b>Step 2/5</b>\n\nTitle: ✅ <b>${s.title}</b>\n\nChoose the task type:`, { reply_markup: { inline_keyboard: [
            ...
            [{ text: '🚰 Faucet Site', callback_data: 'task_cat_faucet' }],
            [{ text: '📰 Article', callback_data: 'task_cat_article' }],
        ] } });
        return res.status(200).json({ ok: true });
    }
