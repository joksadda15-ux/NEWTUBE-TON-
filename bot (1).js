...
export default async function handler(req, res) {
    ...
    if (update.callback_query) {
        ...
        if (data === 'a_sendgift') {
            ...
            await tgEdit(chatId, msgId, '🎁 <b>Send Gift — Step 1/3</b>\n\nকী কারণে গিফট দিচ্ছেন? (এই কারণটা user app খুললে দেখতে পাবে)', { reply_markup: cancelKb });
            return res.status(200).json({ ok: true });
        }
        if (data === 'gift_confirm') {
            ...
            await tgEdit(chatId, msgId, `✅ <b>Gift তৈরি হয়েছে!</b>\n\n👤 <code>${gs.targetId}</code>\n💰 ${gs.amount.toLocaleString()} WTC\n📝 ${gs.reason}\n\nUser app খুললেই animated gift-box দেখবে এবং claim করলে balance যোগ হবে।`, { reply_markup: { inline_keyboard: [[{ text: '◀️ Back to Menu', callback_data: 'a_menu' }]] } });
            tgSend(gs.targetId, `🎁 আপনার জন্য একটা <b>surprise gift</b> অপেক্ষা করছে! এটা দেখতে NEWTUBE app খুলুন।`).catch(() => {});
            return res.status(200).json({ ok: true });
        }
        if (data === 'gift_cancel') {
            await clearAdminState(fromId);
            await tgEdit(chatId, msgId, '❌ Gift বাতিল করা হয়েছে।', { reply_markup: { inline_keyboard: [[{ text: '◀️ Back to Menu', callback_data: 'a_menu' }]] } });
            return res.status(200).json({ ok: true });
        }
    ...
    if (s.step === 'gift_reason') {
        ...
        await setAdminState(fromId, s);
        await tgSend(chatId, `📝 কারণ: <b>${text}</b>\n\n🎁 <b>Send Gift — Step 2/3</b>\n\nকাকে গিফট দিতে চান? <b>Telegram ID</b> অথবা <b>@username</b> দিন:`, { reply_markup: cancelKb });
        return res.status(200).json({ ok: true });
    }
    if (s.step === 'gift_target') {
        ...
        if (!target) { await tgSend(chatId, '❌ User খুঁজে পাওয়া যায়নি। আবার ID/username দিন, অথবা Cancel চাপুন:', { reply_markup: cancelKb }); return res.status(200).json({ ok: true }); }
        s.targetId = target._id; s.targetUsername = target.telegramUsername || 'N/A'; s.step = 'gift_amount';
        await setAdminState(fromId, s);
        await tgSend(chatId, `👤 পাওয়া গেছে: <code>${target._id}</code> (@${s.targetUsername})\n\n🎁 <b>Send Gift — Step 3/3</b>\n\nকত <b>WTC</b> গিফট দিতে চান?`, { reply_markup: cancelKb });
        return res.status(200).json({ ok: true });
    }
    if (s.step === 'gift_amount') {
        const amt = parseInt(text);
        if (!amt || isNaN(amt) || amt <= 0) { await tgSend(chatId, '❌ সঠিক সংখ্যা দিন:'); return res.status(200).json({ ok: true }); }
        s.amount = amt; s.step = 'gift_confirm';
        await setAdminState(fromId, s);
        await tgSend(chatId,
            `🎁 <b>Gift Preview</b>\n\n👤 <code>${s.targetId}</code> (@${s.targetUsername})\n💰 <b>${amt.toLocaleString()} WTC</b>\n📝 ${s.reason}\n\nএটা confirm করলে user app খুললেই animated gift-box দেখবে।`,
