...
//
// ⚠️ REMOVED (this update) — a daily per-referrer milestone-reward cap
...
// actual anti-abuse protection is the referral-SIGNUP velocity lock in
// api/user.js (20+ signups under one referrer within 2 minutes → that
// referrer's account gets accountLocked, checked below) — that's a much
// earlier, more targeted tripwire, and it's the one kept.
//
...
export async function maybeAwardReferralMilestones(db, referredUserId, stats = {}) {
    ...
    for (const step of steps) {
        ...
        const referrerUpdate = await users.findOneAndUpdate(
            // ⚠️ velocity-locked referrer (api/user.js REFERRAL_VELOCITY_*) still
            // gets skipped here — that's the one anti-abuse gate kept.
