...
//
// ⚠️ NEW — `lastSeenAt` is refreshed every time the OWNER's own check passes
// (see checkIp below), and a TTL index on it (models/schema.js) auto-deletes
// the doc after 120 days of that IP going completely quiet. Free-tier
// storage hygiene: most IPs are dynamic (mobile carriers, home routers) and
// get reassigned to a different person eventually — an ipRegistry doc for
// an IP nobody in this app has touched in 4 months has no ongoing value and
...
export async function checkIp(db, ip, userId) {
    ...
    if (entry.userId === userId) {
        // ⚠️ NEW — keep this IP's TTL clock fresh while its real owner is
        ...
        db.collection('ipRegistry').updateOne({ _id: ip }, { $set: { lastSeenAt: new Date() } }).catch(() => {});
