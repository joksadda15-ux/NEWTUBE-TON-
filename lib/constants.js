...
// ⚠️ NEW — anti-farming-bot protection (see api/earn.js's adStart/claimAdReward).
// A claim is only accepted if at least this many seconds have passed since
// the signed ad-token was issued — a genuine ad SDK flow always takes real
// wall-clock time to load+show, so an instant claim right after requesting
// the token means no ad was actually shown.
...
export function dailyResetFields() {
    return {
        ...
        usedAdStarts: [],    // ⚠️ NEW — replay-protection: প্রতিদিন claim করা ad-token গুলোর তালিকা (see api/earn.js claimAdReward), দিন শেষে খালি হয়
    };
}
