...
// ⚠️ SEASON 3 CHANGE — was a LIFETIME gate (completedTasks.length >= 10).
// Now checked against tasksCompletedToday instead (same Bangladesh-day
// counter the ads requirement already uses), per admin's Season 3 spec:
// "8 tasks + 10 ads within 24hr" to withdraw. Kept the old export name out
// — see WITHDRAW_TASKS_REQUIRED below, which replaces it. If anything else
// still imports FIRST_WITHDRAW_MIN_TASKS, update that import too.
export const WITHDRAW_TASKS_REQUIRED = 8; // ⚠️ CHANGED — was 10 lifetime, now 8 TODAY (see above)
...
export const WITHDRAW_ADS_REQUIRED = 10; // ⚠️ CHANGED — was 8, Season 3 spec asks for 10/day
...
export const WITHDRAW_TIERS = [
    { id: 't005', usd: 0.05, monthlyLimit: 5, referralsRequired: 1   }, // ⚠️ CHANGED — was 0 (free withdraw). Every withdraw now needs at least 1 referral, no exceptions.
    { id: 't010', usd: 0.10, monthlyLimit: 5, referralsRequired: 2   }, // ⚠️ CHANGED — was 1 → 2
    { id: 't020', usd: 0.20, monthlyLimit: 5, referralsRequired: 3   }, // ⚠️ CHANGED — was 2 → 3 (fixed the t010/t020 tie)
    { id: 't050', usd: 0.50, monthlyLimit: 3, referralsRequired: 5   },
    { id: 't1',   usd: 1,    monthlyLimit: 3, referralsRequired: 8   }, // ⚠️ CHANGED — was 10 → 8
    { id: 't2',   usd: 2,    monthlyLimit: 3, referralsRequired: 12  }, // ⚠️ CHANGED — was 15 → 12
...
// ══════════════════════════════════════════════════════════
// SEASON 3 — WITHDRAW LEVEL SYSTEM
//
// A SEPARATE gate from WITHDRAW_TIERS above (tiers still decide the $
// amount + their own referral requirement). This one caps how many
// withdrawal ACTIONS (of any tier) a user gets before they must refer more
// people to level up for more attempts.
//
//   - `referralsRequired` — LIFETIME total referrals needed to REACH this
//     level (threshold, not consumed — same semantics as tier.referralsRequired).
//   - `withdrawsAllowed` — how many withdrawals a user can make WHILE at
//     this level, before needing to level up for more.
//   - Level 6 is FINAL (no level 7). Its withdrawsAllowed (10) resets every
//     6 months, same as WITHDRAW_TIERS.monthlyLimit — otherwise a maxed-out
//     user would hit a permanent dead end once those 10 are spent. All
//     other levels are one-way: once spent, leveling up is the only path
//     forward (no periodic reset needed since the next level is always
//     reachable via more referrals).
//   - This whole level ladder is intentionally HIDDEN from the user until
//     they open the withdraw flow — see index.html's withdraw wizard.
// ══════════════════════════════════════════════════════════
export const WITHDRAW_LEVELS = [
    { level: 1, referralsRequired: 0,  withdrawsAllowed: 2  },
    { level: 2, referralsRequired: 5,  withdrawsAllowed: 2  },
    { level: 3, referralsRequired: 10, withdrawsAllowed: 2  },
    { level: 4, referralsRequired: 20, withdrawsAllowed: 2  },
    { level: 5, referralsRequired: 30, withdrawsAllowed: 2  },
    { level: 6, referralsRequired: 50, withdrawsAllowed: 10, final: true },
];
