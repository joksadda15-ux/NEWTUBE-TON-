...
export const AD_NETWORK_REWARDS = {
    ...
    adsgramSpecial: { reward: 20, dailyLimit: 10 },
    monetag:        { reward: 10, dailyLimit: 10 },
...
export function dailyResetFields() {
    return {
        ...
        // ⚠️ NEW — daily free spin pool for the Spin Wheel. Resets to 0 daily
        // (3 free ad-gated spins/day, see SPIN_DAILY_FREE below).
        // bonusSpinsAvailable (referral spins) is intentionally NOT reset
        // here — those accumulate and don't expire daily.
        dailySpinsUsed: 0,
...
// ══════════════════════════════════════════════════════════
// SPIN WHEEL — ⚠️ NEW. 3 free ad-gated spins/day (dailySpinsUsed, resets via
// dailyResetFields above) + 1 bonus spin per referral signup
// (bonusSpinsAvailable, accumulates, never expires — see api/user.js).
// Watching the ad is the "cost" of a spin turn — same trust model as every
// other ad network here (client awaits the SDK's show-promise, we don't get
// independent proof), which is why this doesn't try to be more secure than
// e.g. GigaPub already is. The reward itself comes from SPIN_SEGMENTS below,
// picked by handleSpin in api/earn.js.
//
// Weights implement the admin's spec: {10,20} share 95% combined, {30,40,50}
// share 4% combined, {60,80,100} share the remaining 1% combined. Exact
// per-value split within each group wasn't specified, so split evenly
// within each group — flag if a different split was actually intended.
export const SPIN_DAILY_FREE = 3;
