...
// ⚠️ NEW — daily circuit-breaker on referral-milestone payouts. Every step
// of REFERRAL_REWARDS above is gated by REAL server-side checks (actual
// Telegram channel membership, signed task-claim tokens, real ad-watch
// counters) — there's no bypass bug to close there. But those checks only
// prove "a real Telegram account did this action", not "these 356 accounts
// are 356 different humans" — a script can trivially spin up many cheap
// accounts, each with a different (client-generated, easily randomized)
// device fingerprint, and drive them through the real steps. That's not a
// patchable bug, it's a volume-abuse problem — so the fix here is a hard
// ceiling instead: at most this many referral-milestone rewards get paid
// to any ONE referrer per calendar day (Bangladesh time). Past the cap,
// further milestone credits for that referrer are BLOCKED (not queued —
// simply not paid) for the rest of the day, and the admin gets a one-time
// alert to review the account. Tune this number up if it's blocking
// legitimate viral growth days for real cross-promotion partners.
export const REFERRAL_DAILY_MILESTONE_CAP = 15;
