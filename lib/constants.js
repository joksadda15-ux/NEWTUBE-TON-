...
// time).
//
// ⚠️ FIXED (was accidentally 15 — a prior fix for a missing-export crash
// re-added this constant but with the wrong value). 5 seconds matches what
// was actually agreed: fast enough that a genuine ad view always clears it
// comfortably, without forcing users to sit through a long artificial wait.
...
// ⚠️ CHANGED (was 15) — lowered to 8 per admin request: 15 ads/day was too
// hard a gate to clear before withdrawing. Fixed ad-watch requirement per
// withdraw REQUEST (not per tier, not scaled by amount). Must be watched
// within the current Bangladesh calendar day (same day-boundary as every
// other daily counter in this app — see todayBD()/dailyResetFields() below).
...
//     same 8 ads (today) + 10 tasks (lifetime). Only referralsRequired and
//     monthlyLimit differ by tier.
//
//   ⚠️ NOT touched in this update — these referralsRequired values were
//   changed in a prior update (not part of today's fix). Left exactly as
//   received; let me know if you'd like these adjusted too.
// ══════════════════════════════════════════════════════════
export const WITHDRAW_TIERS = [
    { id: 't005', usd: 0.05, monthlyLimit: 5, referralsRequired: 1   },
    { id: 't010', usd: 0.10, monthlyLimit: 5, referralsRequired: 2   },
    { id: 't020', usd: 0.20, monthlyLimit: 5, referralsRequired: 3   },
    { id: 't050', usd: 0.50, monthlyLimit: 3, referralsRequired: 5   },
    { id: 't1',   usd: 1,    monthlyLimit: 3, referralsRequired: 8   },
    { id: 't2',   usd: 2,    monthlyLimit: 3, referralsRequired: 12  },
...
export function dailyResetFields() {
    return {
        ...
        usedAdStarts: [],    // ⚠️ replay-protection: প্রতিদিন claim করা ad-token গুলোর তালিকা (see api/earn.js claimAdReward), দিন শেষে খালি হয়
    };
}
