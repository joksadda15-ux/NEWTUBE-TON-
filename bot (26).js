...
function renderUserLookup(u, withdrawalsList = []) {
    ...
    const text =
        ...
        // ⚠️ NEW — surfaces the referral-velocity auto-lock (lib/constants.js
        // REFERRAL_VELOCITY_*) so the admin can eyeball it right alongside
        // Banned and decide: Unlock (false alarm) or Ban (confirmed fraud).
    ...
    const options = { reply_markup: { inline_keyboard: [
        ...
        // ⚠️ NEW — only shown when locked, so it doesn't clutter the panel
        // for the vast majority of accounts that never trip the velocity check.
        ...(u.accountLocked ? [[{ text: '🔓 Unlock Account', callback_data: `unlock_${u._id}` }]] : []),
