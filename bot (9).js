    ...
    if (update.callback_query) {
        ...
        if (data.startsWith('wd_approve_') || data.startsWith('wd_reject_')) {
            ...
            if (!approve) {
                ...
                // ⚠️ FIX: previously this didn't refund the tier's claim slot (withdrawTierCounts),
                // so a rejected withdrawal would still permanently burn one of the user's limited
                // monthly claims for that tier even though they got nothing. Now that's refunded too.
                const refundUpdate = { $inc: { wtcBalance: w.wtcAmount, withdrawalCount: -1 }, $set: { lastWithdrawDate: '' } };
