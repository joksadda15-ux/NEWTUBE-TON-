...
async function handleAdStart(req, res, db, userId) {
    ...
    if (!AD_NETWORK_REWARDS[network]) return res.status(400).json({ ok: false, error: 'invalid_network' });
    if (!SECRET) return res.status(500).json({ ok: false, error: 'server_misconfigured' });
...
async function handleClaimAdReward(req, res, db, userId) {
    ...
    if (!gate) {
        const exists = await users.findOne({ _id: userId }, { projection: { isBanned: 1, multiAccountFlag: 1, channelVerified: 1, usedAdStarts: 1 } });
        if (!exists) return res.status(404).json({ ok: false, error: 'user_not_found' });
        if (exists.isBanned) return res.status(403).json({ ok: false, error: 'banned' });
...
// ── lotterySpin — Season 3 "777" game. Spends 1 luckyTicket, server rolls
// the outcome (see rollLottery), credits any reward atomically. Same
// REWARD_ELIGIBLE_FILTER gate as every other reward path — but the spin
// still consumes the ticket even if a flagged/unverified account can't
// collect the reward this time, matching claimPromo's existing trade-off
// above (prevents free unlimited retries while under review). ──
async function handleLotterySpin(req, res, db, userId) {
    const users = db.collection('users');

    const gate = await users.findOneAndUpdate(
        { _id: userId, isBanned: { $ne: true }, luckyTickets: { $gte: LOTTERY_SPIN_COST_TICKETS } },
        { $inc: { luckyTickets: -LOTTERY_SPIN_COST_TICKETS } },
        { returnDocument: 'after' }
    );
    if (!gate) return res.status(400).json({ ok: false, error: 'insufficient_tickets' });

    const outcome = rollLottery();

    if (outcome.reward > 0) {
        const creditResult = await users.updateOne(
            { _id: userId, ...REWARD_ELIGIBLE_FILTER },
            { $inc: { wtcBalance: outcome.reward, lifetimeWtcEarned: outcome.reward } }
        );
        if (creditResult.matchedCount === 0) {
            // spin already consumed the ticket — reward blocked pending verification, same as claimPromo
            return res.status(200).json({ ok: true, reels: outcome.reels, reward: 0, ticketsLeft: gate.luckyTickets, blocked: true });
        }
    }

    return res.status(200).json({ ok: true, reels: outcome.reels, reward: outcome.reward, ticketsLeft: gate.luckyTickets });
}
...
export default async function handler(req, res) {
    ...
    switch (action) {
        ...
        case 'lotterySpin':    return handleLotterySpin(req, res, db, userId);
