...
//      Enforcement is now IP-based (see lib/ipRegistry.js) and happens live
//      at the door, not via a manual admin queue — this panel was fully
//      redundant with that and is gone. Ban/unban of an individual user
//      (ban_/unban_ callbacks) is untouched.
    ...
    if (update.callback_query) {
        ...
        if (data.startsWith('wd_approve_') || data.startsWith('wd_reject_')) {
            ...
            if (!approve) {
                ...
                const refundUpdate = { $inc: { usdtBalance: w.cashAmount, withdrawalCount: -1 }, $set: { lastWithdrawDate: '', withdrawPending: false } };
            ...
            const processedText =
                ...
                // ⚠️ FIX: was `${w.wtcAmount} WTC (fee ${w.feeWtc})` — feeWtc doesn't
                // exist in the Season 3 schema, so this crashed on every single
                // approve/reject click (undefined.toLocaleString()), which is why
                // the buttons looked completely dead from the admin's side.
                `💰 Amount: <b>${w.cashAmount.toFixed(4)} ${w.currency}</b>\n` +
                `📤 Method: <b>${w.method}</b>\n` +
        ...
        if (data === 'a_pending') {
            ...
            for (const w of list) {
                await tgSend(chatId,
                    ...
                    `👤 <code>${w.userId}</code> (@${w.username || '?'})\n` +
                    (w.tierId ? `🎯 Tier: <b>${w.tierId}</b>\n` : '') +
                    // ⚠️ FIX: was `${w.wtcAmount} WTC (fee ${w.feeWtc})` — feeWtc was
                    // never set anywhere in the Season 3 convert-first schema, so this
                    // threw on undefined.toLocaleString() every time, which is why the
                    // whole pending-withdrawals list rendered nothing after the first item.
                    `💰 Amount: <b>${w.cashAmount.toFixed(4)} ${w.currency}</b>\n` +
                    `📤 Method: <b>${w.method}</b>\n` +
                    `📍 Address: <code>${w.details}</code>\n` +
                    `📅 ${new Date(w.createdAt).toLocaleString()}`,
