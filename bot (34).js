...
export default async function handler(req, res) {
    ...
    if (update.callback_query) {
        ...
        // code is generated (see promo_maxuses below — only offered when a
        // single code was created, since broadcasting only makes sense for
        // one shared code). Pre-fills the broadcast text with a ready-made
        // description + the code, then drops into the normal broadcast flow
        // from Step 2/4 onward so the admin still gets the usual
        // button/photo/preview steps before anything actually sends.
    ...
    if (s.step === 'promo_maxuses') {
        ...
        // ⚠️ NEW — "Broadcast This Code" button only when exactly one code
        // was generated. Broadcasting only makes sense for a single code
        // shared with many users (typically paired with a high/unlimited
        // max-uses) — a batch of individual single-use codes (e.g. giveaway
        // winners) has no single code to announce, so no button for count > 1.
        const kb = s.count === 1
            ? { inline_keyboard: [[{ text: '📢 Broadcast This Code', callback_data: `pbc_${generated[0]}` }], [{ text: '◀️ Back to Menu', callback_data: 'a_menu' }]] }
            : adminKb;
        await tgSend(chatId,
            `✅ <b>${s.count} Promo Code(s) created!</b>\n\n💰 Reward: <b>${s.reward} WTC</b> each\n👥 Max uses: <b>${maxUses}</b>\n⏰ Expires: <b>24 hours</b>\n\n📋 <b>Codes:</b>\n${codeList}\n\n⚠️ Use the <b>Broadcast-safe</b> version when pasting into a broadcast message — it looks identical to users but resists simple copy-paste sharing.`,
            { reply_markup: kb }
        );
        return res.status(200).json({ ok: true });
    }
