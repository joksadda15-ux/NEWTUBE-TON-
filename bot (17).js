...
import { WEEKLY_REFERRAL_MIN_COUNT, WEEKLY_REFERRAL_MAX_WINNERS, WTC_PER_USD, WITHDRAW_REFERRAL_COMMISSION_PERCENT, fmtBDDateTime, fmtBDDate } from '../lib/constants.js';
...
export default async function handler(req, res) {
    ...
    if (update.callback_query) {
        ...
        // ⚠️ NEW — user's one-time free chance to correct their address
        // instead of eating the 50% penalty. Only offered if this specific
        // withdrawal hasn't already used its one fix (w.addressFixUsed).
        ...
        if (data.startsWith('wd_fixaddr_')) {
            ...
            if (w.addressFixUsed) {
                ...
                await tgAnswerCallback(cb.id, "You've already used your one free address fix for this withdrawal.", true);
                return res.status(200).json({ ok: true });
            }
            ...
            await tgSend(chatId, `✏️ Please reply with your correct <b>${w.method === 'binance' ? 'Binance UID' : 'Tonkeeper Address'}</b> now.\n\nYou get <b>one free fix</b> — if this one is wrong too, only the 50%-penalty option will be available.`);
        ...
        if (data.startsWith('wd_wrongaddr_')) {
            ...
            // ⚠️ CHANGED — offers a free "Fix My Address" button too, but ONLY
            // if this withdrawal hasn't already used its one free fix
            // (w.addressFixUsed). Once used, only the 50%-penalty confirm
            // remains — this is what makes it a genuine one-time allowance
            // rather than an unlimited do-over.
            const fixButtons = w.addressFixUsed
                ? []
                : [[{ text: '✏️ Fix My Address (one free fix)', callback_data: `wd_fixaddr_${wid}` }]];
            await tgSend(
                w.userId,
                ...
                (w.addressFixUsed
                    ? `You've already used your one free address fix on this withdrawal, so this can only be closed with a 50% penalty now — please double-check your address next time.`
                    : `You can fix your address for free (one time) below, or confirm to close this request now — <b>50% of the WTC will be refunded to your balance, and 50% will be kept as a penalty</b> for submitting an invalid address.`),
                { reply_markup: { inline_keyboard: [
                    ...fixButtons,
    ...
    // ⚠️ NEW — a regular (non-staff) user replying with their corrected
    // withdrawal address after tapping "✏️ Fix My Address" (wd_fixaddr_
    // above). Deliberately checked here, BEFORE both the /start handling
    // below and the staff-only gate further down — this has to work for
    // ANY user, not just admin/task-moderator.
    {
        ...
        if (addrState && addrState.step === 'wd_new_address') {
            ...
            if (!w || w.status !== 'pending' || String(w.userId) !== fromId || w.addressFixUsed) {
                await tgSend(chatId, "⚠️ This withdrawal can no longer be updated (it may have already been processed, or the free fix was already used).");
                return res.status(200).json({ ok: true });
            }
            ...
            await withdrawals.updateOne(
                { _id: w._id },
                { $set: { details: text, addressFixUsed: true, addressFixedAt: new Date() } }
            );
            await tgSend(chatId, `✅ Address updated to:\n<code>${text}</code>\n\nYour withdrawal is back under review — you'll be notified once it's processed.`);
            if (ADMIN_ID) {
                await tgSend(ADMIN_ID,
                    `✏️ <b>User Corrected Their Address</b>\n\n` +
                    `👤 User: <code>${fromId}</code>\n` +
                    `🪙 WTC: <b>${(w.wtcAmount || 0).toLocaleString()}</b>\n` +
                    `📍 New address: <code>${text}</code>\n` +
                    `🆔 Request: <code>${w._id}</code>\n\n` +
                    `Please re-review — this was their one free fix, so a second wrong address here can only be closed with the 50% penalty.`,
                    { reply_markup: { inline_keyboard: [
                        [{ text: '✅ Approve', callback_data: `wd_approve_${w._id}` },
                         { text: '❌ Reject', callback_data: `wd_reject_${w._id}` }],
                        [{ text: '⚠️ Wrong Address', callback_data: `wd_wrongaddr_${w._id}` }],
                    ] } }
                ).catch(() => {});
            }
            return res.status(200).json({ ok: true });
